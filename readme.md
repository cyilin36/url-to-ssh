...
1. **解析 URL**：从 URL 路径中获取要执行的 **命令**，从 URL 参数中获取目标设备的 **IP、用户名、密码**。
2. **建立连接**：使用 Paramiko 库建立到目标设备的 SSH 连接（支持自定义 SSH 端口，默认 22）。
3. **执行与返回**：在目标设备上执行命令，并将标准输出（stdout）和错误输出（stderr）合并，以 **纯文本 (text/plain)** 格式直接返回给浏览器或调用端。
